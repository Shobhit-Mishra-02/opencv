...
In this GUI we can select a image, then we can change their background color and also we edit the contrast
of the selected image.
With this GUI we can actually merge two images and also we can produce a black & white image.
'''
...
class main_interace:
    ...
    def start_creation(self):
        ...
        # In these five lines which have been written below, I have updated the status bar,
        ...
        # This function will add the processed image in the image slot.
        ...
        # Here I have written below nine function for the options of the submenue.


        # This function opens the directory and ask to select the image and 
        # then it placed the selected image in the slot image position.
        def openfile():
        ...
        # This function allows the user to save the edited image.
        def savefile():
            if type(self.processed_image) == np.ndarray:
                ...
                file_saver = image_processor.save_file(
                    image=self.processed_image, location=location, width=self.var_width.get(), height=self.var_height.get())
                file_saver.save_image()
            else:
                msg.showerror(
                    'Error', 'First select the image from the open file option.')

        # This function simply destroy the main interface windo.
        def Exit():
            self.root.destroy()

        # This function will allow only contrast labled frame to enable.
        def contrast():
        ...
        # This function will allow only advanced color labeled frame to be enabled.
        def rbg():
            if self.filename != None:
        ...
        # This function will allow only merging labeled frame to be enabled.
        def merging_effect():
            if self.filename != None:
        ...
        # This function will allow only size changing labeled frame to be enabled.
        def size_change():
            if self.filename != None:
        ...
        # This will allow only control labled frame to be enabled.
        def simple_shade():
            if self.filename != None:
                if control_rbg.instate(['disabled']) == False:
                    control_rbg.state(['!disabled'])
        ...
        # This function displays a message about the application.
        def about():
        ...
        # This labeled frame will hold some selected colors.
        control_frame = ttk.LabelFrame(self.root, text='Color controls')
        control_frame.grid(row=0, column=0, columnspan=2, sticky=['n', 'e', 'w', 's'])

        # Now here I have created a list of empty variables, their labels and placing them in the control frame.
        gray_button = None
        red_button = None
        blue_button = None
        green_button = None
        ...
        # placing two entry widgets and apply button in the sizing frame.
